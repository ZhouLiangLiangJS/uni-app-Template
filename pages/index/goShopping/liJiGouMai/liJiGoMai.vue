			...
			<view>
				...
				<view>
					<view class="shopXinXi">
						<image class="shopXinXi_img" :src="url+'/image/'+goShopping.img1[0]" mode="aspectFill"></image>
						<view class="shopXinXi_txt">{{goShopping.name}}</view>
						<view class="shopXinXi_fenLei">
							<text v-for="(item,i) in txt" :key='i'>{{item.title}}</text>
						</view>
						<view class="jiage">
							<text>￥{{jg+goShopping.freight}}</text>
							<text>${{jg+goShopping.freight}}</text>
						</view>
						<view class="shuliang">
							× 1
				...
				<view class="heji">
					<view class="fr">￥{{jg+goShopping.freight}}</view>
				...
				<view class="liuYan">
					<textarea v-model="liuyan" placeholder="买家留言"  />
				</view>
			</view>
		</view>
		<view class="diBu">
			<view >总计：￥{{jg+goShopping.freight}}</view>
...
<script>
	export default {
		...
		onLoad(op) {
			this.goShopping=JSON.parse(op.goShopping)
			this.shop=JSON.parse(op.shop)
			this.txt=JSON.parse(op.txt)
			for(let i=0;i<this.txt.length;i++){
				this.jg+=new Number(this.txt[i].jg)
			}
			
			console.log(typeof this.goShopping.freight,this.jg)
		},
		onShow(){
			this.req()
		},
				...
				if(this.address.length>0){
					...
					uni.request({
						...
						data: {
							...
							shopOrderNum:1,
							shopPrice:that.jg,
							shopFreight:that.goShopping.freight,
							message:that.liuyan,
							Coupon:0,
						...
						if(res.statusCode==200){
							uni.request({
								...
								data: {
									uid:that.useUser.uid,
									sid:that.shop.uid,
									money:that.jg+that.goShopping.freight
								},
							success: (res) => {
								console.log(res)
								if(res.statusCode==200){
