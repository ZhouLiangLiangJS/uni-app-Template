<template>
	<view>
		<view class="hader-box">
					 ...
					 <text class="hader-box-text">施工中</text>
		</view>
		
		<view style="margin-top: 200upx;">
			<view class="main-box" @click="showgn" v-for="(item,i) in listArr" :key="i" v-if="item.workover==0?item.num>=item.people?item.workover==0?true:false:false:false">
				...
				<view class="main-box-bottom" @click="yiwancheng(item,i)" style=" background-color: #ff6900; width: 100%;border-radius:20upx ;" >
					如已完成请点击此处
				</view>
			</view>
		</view>
...
<script>
	export default {
		...
		methods: {
			yiwancheng:function(item,i){
				
				let that=this;
					let usurl0=that.url1+"/dinghai/order/updateWorkover";
				uni.showModal({
				        title: '此单是否已经完成？',
